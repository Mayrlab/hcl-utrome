...
if (interactive()) {
    ...
    snakemake <- Snakemake(
        input=list(granges="data/granges/augmented.plus.chunked.e3.t10.gc39.pas3.f0.9999.Rds"),
        output=list(granges="/fscratch/fanslerm/utrome.raw.plus.Rds"),
        wildcards=list(epsilon="3", threshold="200", version="39", tpm="3",
                       likelihood="0.9999", width=500, chunk="101"),
        params=list(),
        threads=3
        )
}
...
grl_plus <- readRDS(snakemake@input$granges)
...
gr_plus_adjusted <- bplapply(grl_plus[as.integer(snakemake@wildcards$chunk)], function (gr) {
...
saveRDS(gr_plus_adjusted, snakemake@output$granges)
