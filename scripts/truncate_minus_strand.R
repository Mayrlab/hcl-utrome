...
gr_minus_adjusted <- bplapply(grl_minus, function (gr) {
    ...
    gr_exons_adjusted <- gr_children %>%
        ...
        mutate(tx_length_start=cumsum(width) - width,
               tx_length_end=tx_length_start + width) %>%
        ## exclude exons beyond max tx length
        filter(tx_length_start < MAX_TX_LENGTH) %>%
        ## adjust all exons traversing cutoff
        mutate(end=ifelse(tx_length_end > MAX_TX_LENGTH, end - tx_length_end + MAX_TX_LENGTH, end)) %>%
        ungroup()
